...
def move_ur30_base_trajectory():
    ...
    rospy.init_node('control_ur30_velocity', anonymous=True)
    ...
    point2.positions = [3.1415, -0.78, -1.57, -0.78, 0.0, 0.0]
    point2.time_from_start = rospy.Duration(6.0)  # Temps total pour le mouvement (3s + 3s = 6s)

    # --- Point 3 : Rotation de la base de 90 degrés ---
    point3 = JointTrajectoryPoint()
    point3.positions = [3.9215, -0.78, 0.0, -0.78, 0.0, 0.0]
    point3.time_from_start = rospy.Duration(9.0)  # Temps total pour le mouvement (3s + 3s = 6s)

    
    # Ajoute les points à la trajectoire
    traj_msg.points.append(point1)
    traj_msg.points.append(point2)
    traj_msg.points.append(point3)
